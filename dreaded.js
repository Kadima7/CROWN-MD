...
const botname = process.env.BOTNAME || '𝐋𝐎𝐍𝐄𝐑 𝐀𝐈';
const antibot = process.env.ANTIBOT || 'FALSE';
    ...
    const author = process.env.STICKER_AUTHOR ||'⇆࿅𝐈𝐀𝐍࿅⇆↯';
...
const admin = process.env.ADMIN_MSG || '𝐅𝐚𝐢𝐥𝐮𝐫𝐞!, 𝐘𝐨𝐮 𝐚𝐫𝐞 𝐭𝐨𝐨 𝐲𝐨𝐮𝐧𝐠 𝐭𝐨 𝐝𝐨 𝐭𝐡𝐚𝐭🦄';
    ...
    const botAdmin = process.env.BOT_ADMIN_MSG || '👽𝐚𝐦 𝐚𝐧 𝐧𝐨𝐭 𝐚𝐧 𝐚𝐝𝐦𝐢𝐧'
    const NotOwner = process.env.NOT_OWNER_MSG || 'Lol you are not ⇆࿅𝐈𝐀𝐍࿅⇆↯';
...
if (antilink === 'TRUE' && antilinkall === 'TRUE' && body.includes('http') && !Owner && isBotAdmin && !isAdmin && m.isGroup) { 
 ...
 client.sendMessage(m.chat, {text:`Auf wiedersehen:\n\n@${ki.split("@")[0]}, sending links is prohibited in this group!`, contextInfo:{mentionedJid:[ki]}}, {quoted:m}); 
    ...
    if (cmd && !m.isGroup) {
      console.log(chalk.black(chalk.bgWhite("[ _𝐋𝐎𝐍𝐄𝐑-AI ]")), color(argsLog, "turquoise"), chalk.magenta("From"), chalk.green(pushname), chalk.yellow(`[ ${m.sender.replace("@s.whatsapp.net", "")} ]`));
...
client.sendMessage(m.chat, {
                        ...
                        contextInfo: {
                            externalAdReply: {
                                ...
                                sourceUrl: 'https://wa.me/254793971348?text=ʜᴇʟʟᴏ+⇆࿅𝐈𝐀𝐍࿅⇆↯+Nihostie+Bot+Mkuu+👽',
                                mediaType: 1,
...
try {
  ...
  let encmediax = await client.sendImageAsSticker(m.chat, mediax, m, { packname:𝐋𝐎𝐍𝐄𝐑 𝐀𝐈 𝐁𝐎𝐓 , author: 🕷️®️⇆࿅𝐈𝐀𝐍࿅⇆↯®️🕷️ });
  ...
  let encmediaxx = await client.sendVideoAsSticker(m.chat, mediaxx, m, { packname:𝐋𝐎𝐍𝐄𝐑 𝐀𝐈 𝐁𝐎𝐓 , author: 🕷️®️⇆࿅𝐈𝐀𝐍࿅⇆↯®️🕷️ });
...
case "alive": { 
 ...
 client.sendMessage(m.chat, { video: { url: 'https://i.imgur.com/IbM7XSM.mp4' }, caption: `Hello ${m.pushName},𝐋𝐎𝐍𝐄𝐑 𝐀𝐈 is alive since  ${runtime(process.uptime())}`, fileLength: "9999999999898989899999999" }, { quoted: m }); 
 }
break;
case 'apk': {
...
client.sendMessage(from, buttonMessage, { quoted: m }) 
 ...
 break 
   ...
   client.sendMessage(m.chat, { image: buffer, caption: `*_𝐋𝐎𝐍𝐄𝐑 𝐀𝐈 𝐂𝐨𝐧𝐯𝐞𝐫𝐭𝐞𝐫 ®️🕷️_*`}, { quoted: m }) 
...
case "movie": 
 ...
 client.sendMessage(m.chat, { video: { url: 'https://i.imgur.com/9WyMGc4.mp4' }, caption: `Say no more 😌 here is ⇆࿅𝐈𝐀𝐍࿅⇆↯ contact +254793971348 🥱` }, {quoted: m}); 
...
async function handleGPTMessage(text, m) {
  if (!text) return m.reply("Hello am 𝐋𝐎𝐍𝐄𝐑 𝐀𝐈  an Ai developed by ⇆࿅𝐈𝐀𝐍࿅⇆↯, how can I help you today?");
